...
namespace meta {
	...
	public:
	  ...
	  bool hasCatalogName() const { return m_pMetadata->has_catalog_name(); }
	  const std::string& getCatalogName() const { return m_pMetadata->catalog_name(); }

	  bool hasDescription() const { return m_pMetadata->has_description(); }
	  const std::string& getDescription() const { return m_pMetadata->description(); }

	  bool hasConnect() const { return m_pMetadata->has_connect(); }
	  const std::string& getConnect() const { return m_pMetadata->connect(); }

	private:
		const ::exec::user::CatalogMetadata* m_pMetadata;
	};
	...
	public:
		...
		bool hasSchemaName() const { return m_pMetadata->has_schema_name(); }
		const std::string& getSchemaName() const { return m_pMetadata->schema_name(); }

		bool hasOwnerName() const { return m_pMetadata->has_owner(); }
		const std::string& getOwner() const { return m_pMetadata->owner(); }

		bool hasType() const { return m_pMetadata->has_type(); }
		const std::string& getType() const { return m_pMetadata->type(); }

		bool hasMutable() const { return m_pMetadata->has_mutable_(); }
		const std::string& getMutable() const { return m_pMetadata->mutable_(); }

	private:
		const ::exec::user::SchemaMetadata* m_pMetadata;
	};
	...
	public:
	  ...
	  bool hasTableName() const { return m_pMetadata->has_table_name(); }
	  const std::string& getTableName() const { return m_pMetadata->table_name(); }

	  bool hasType() const { return m_pMetadata->has_type(); }
	  const std::string& getType() const { return m_pMetadata->type(); }

	private:
		const ::exec::user::TableMetadata* m_pMetadata;
	};
	...
	public:
		...
		bool hasColumnName() const { return m_pMetadata->has_column_name(); }
		const std::string& getColumnName() const { return m_pMetadata->column_name(); }

		bool hasOrdinalPosition() const { return m_pMetadata->has_ordinal_position(); }
		std::size_t getOrdinalPosition() const { return m_pMetadata->ordinal_position(); }

		bool hasDefaultValue() const { return m_pMetadata->has_default_value(); }
		const std::string& getDefaultValue() const { return m_pMetadata->default_value(); }

		bool hasNullable() const { return m_pMetadata->has_is_nullable(); }
		bool isNullable() const { return m_pMetadata->is_nullable(); }

		bool hasDataType() const { return m_pMetadata->has_data_type(); }
		const std::string& getDataType() const { return m_pMetadata->data_type(); }

		bool hasCharMaxLength() const { return m_pMetadata->has_char_max_length(); }
		std::size_t getCharMaxLength() const { return m_pMetadata->char_max_length(); }

		bool hasCharOctetLength() const { return m_pMetadata->has_char_octet_length(); }
		std::size_t getCharOctetLength() const { return m_pMetadata->char_octet_length(); }

		bool hasNumericPrecision() const { return m_pMetadata->has_numeric_precision(); }
		int32_t getNumericPrecision() const { return m_pMetadata->numeric_precision(); }

		bool hasNumericRadix() const { return m_pMetadata->has_numeric_precision_radix(); }
		int32_t getNumericRadix() const { return m_pMetadata->numeric_precision_radix(); }

		bool hasNumericScale() const { return m_pMetadata->has_numeric_scale(); }
		int32_t getNumericScale() const { return m_pMetadata->numeric_scale(); }

		bool hasIntervalType() const { return m_pMetadata->has_interval_type(); }
		const std::string& getIntervalType() const { return m_pMetadata->interval_type(); }

		bool hasIntervalPrecision() const { return m_pMetadata->has_interval_precision(); }
		int32_t getIntervalPrecision() const { return m_pMetadata->interval_precision(); }

	private:
		const ::exec::user::ColumnMetadata* m_pMetadata;
	};
    ...
    public:
        ...
        bool isConvertSupported(common::MinorType from, common::MinorType to) const { return false; }
